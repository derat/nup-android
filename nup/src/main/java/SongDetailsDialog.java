...
class SongDetailsDialog {
  ...
  public static void prepareDialog(Dialog dialog, Bundle bundle) {
    ((TextView) dialog.findViewById(R.id.artist)).setText(bundle.getString(BUNDLE_ARTIST));
    ((TextView) dialog.findViewById(R.id.title)).setText(bundle.getString(BUNDLE_TITLE));
    ((TextView) dialog.findViewById(R.id.album)).setText(bundle.getString(BUNDLE_ALBUM));
    ((TextView) dialog.findViewById(R.id.track))
        .setText(Integer.toString(bundle.getInt(BUNDLE_TRACK_NUM)));
    ((TextView) dialog.findViewById(R.id.length))
        .setText(Util.formatDurationString(bundle.getInt(BUNDLE_LENGTH_SEC)));
    ...
    if (rating >= 0.0) {
      ...
      for (int i = 1; i <= 5; ++i) ratingStr += (i <= numStars) ? "\u2605" : "\u2606";
    }
