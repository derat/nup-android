...
internal class FileCacheDatabase(context: Context, private val musicDir: String) {
    ...
    fun addEntry(songId: Long): FileCacheEntry {
        ...
        updater.postUpdate(
                "REPLACE INTO CacheEntries (SongId, TotalBytes, LastAccessTime) VALUES(?, 0, ?)", arrayOf<Any>(songId, accessTime))
        return entry
    }
    ...
    fun setTotalBytes(songId: Long, totalBytes: Long) {
        ...
        updater.postUpdate(
                "UPDATE CacheEntries SET TotalBytes = ? WHERE SongId = ?", arrayOf<Any>(totalBytes, songId))
    }
    ...
    companion object {
        ...
        private const val CREATE_CACHE_ENTRIES_SQL = ("CREATE TABLE CacheEntries ("
        ...
        val helper: SQLiteOpenHelper = object : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
            ...
            private fun upgradeFromPreviousVersion(db: SQLiteDatabase, newVersion: Int) {
                if (newVersion == 2) {
                    ...
                    db.execSQL(
                            "CREATE TABLE CacheEntries (  CacheEntryId INTEGER PRIMARY KEY"
                                    + " AUTOINCREMENT NOT NULL,   RemotePath VARCHAR(2048)"
                                    + " UNIQUE NOT NULL,   LocalPath VARCHAR(2048) UNIQUE NOT"
                                    + " NULL,   ContentLength INTEGER,   ETag VARCHAR(40),  "
                    ...
                    while (!cursor.isAfterLast) {
                        ...
                        val songId = Integer.valueOf(file.name.split("\\.").toTypedArray()[0]).toLong()
                        db.execSQL("REPLACE INTO CacheEntries (SongId, TotalBytes,"
                                + " LastAccessTime) VALUES(?, ?, ?)", arrayOf<Any>(songId, cursor.getInt(1), cursor.getInt(2)))
                        cursor.moveToNext()
                    }
                    cursor.close()
                    db.execSQL("DROP TABLE CacheEntriesTmp")
