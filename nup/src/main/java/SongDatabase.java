...
public class SongDatabase {
        ...
        SQLiteOpenHelper helper =
                new SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
                    ...
                    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
                        ...
                        db.beginTransaction();
                        try {
                            for (int nextVersion = oldVersion + 1;
                                    nextVersion <= newVersion;
                                    ++nextVersion) upgradeFromPreviousVersion(db, nextVersion);
                    ...
                    private void upgradeFromPreviousVersion(SQLiteDatabase db, int newVersion) {
                        if (newVersion == 2) {
                            ...
                            throw new RuntimeException(
                                    "Got request to upgrade database to unknown version "
                                            + newVersion);
                        }
                    }
                };
    ...
    public List<Song> query(
        ...
        String query =
                "SELECT s.SongId, Artist, Title, Album, AlbumId, Url, CoverUrl, Length, "
                        ...
                        + (shuffle
                                ? "RANDOM()"
                                : "Album ASC, AlbumId ASC, DiscNumber ASC, TrackNumber ASC")
                        + " "
                        + "LIMIT "
                        + MAX_QUERY_RESULTS;
        Log.d(
    ...
    public List<PendingPlaybackReport> getAllPendingPlaybackReports() {
        ...
        while (!cursor.isAfterLast()) {
            reports.add(
                    new PendingPlaybackReport(
                            cursor.getLong(0), new Date(cursor.getLong(1) * 1000)));
            cursor.moveToNext();
        }
        cursor.close();
        return reports;
            ...
            try {
                ...
                for (int i = 0; i < objects.length(); ++i) {
                    ...
                    if (deleted) {
                        ...
                        values.put(
                                "CoverUrl",
                                jsonSong.has("coverUrl") ? jsonSong.getString("coverUrl") : "");
                        ...
                        values.put(
                                "AlbumId",
                                jsonSong.has("albumId") ? jsonSong.getString("albumId") : "");
                        ...
                        values.put(
                                "TrackGain",
                                jsonSong.has("trackGain") ? jsonSong.getDouble("trackGain") : 0.0);
                        values.put(
                                "AlbumGain",
                                jsonSong.has("albumGain") ? jsonSong.getDouble("albumGain") : 0.0);
                        values.put(
                                "PeakAmp",
                                jsonSong.has("peakAmp") ? jsonSong.getDouble("peakAmp") : 0.0);
    ...
    private void loadAggregateData(boolean songsUpdated) {
        ...
        while (!cursor.isAfterLast()) {
            artistsSortedAlphabetically.add(
                    new StatsRow(cursor.getString(0), "", "", cursor.getInt(1)));
            cursor.moveToNext();
        }
        cursor.close();
