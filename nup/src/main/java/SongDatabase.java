...
public class SongDatabase {
  ...
  private static final String CREATE_SONGS_ALBUM_INDEX_SQL = "CREATE INDEX Album ON Songs (Album)";
  ...
  public SongDatabase(
    ...
    SQLiteOpenHelper helper =
        new SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
          ...
          // The only reason I'm keeping the old upgrade steps is for reference when writing new
          // ones.
          private void upgradeFromPreviousVersion(SQLiteDatabase db, int newVersion) {
            if (newVersion == 2) {
              ...
              // Version 4: Create ArtistAlbumStats table and indexes on Songs.Artist and
              // Songs.Album.
              db.execSQL(
                  "CREATE TABLE ArtistAlbumStats ("
                      + "  Artist VARCHAR(256) NOT NULL, "
              ...
              db.execSQL(
                  "UPDATE LastUpdateTimeTmp SET MaxLastModified = MaxLastModified * 1000000 WHERE"
                      + " MaxLastModified > 0");
              db.execSQL(CREATE_LAST_UPDATE_TIME_SQL);
              ...
              db.execSQL(
                  "CREATE TABLE Songs ("
                      ...
                      + "  Rating FLOAT NOT NULL)");
              db.execSQL(
                  "INSERT INTO Songs SELECT SongId, Filename, Artist, Title, Album, TrackNumber,"
                      + " Length, Rating FROM SongsTmp WHERE Deleted = 0");
              db.execSQL("DROP TABLE SongsTmp");
  ...
  public List<Song> query(
    ...
    class QueryBuilder {
      ...
      public void add(String clause, String selectionArg, boolean substring) {
        ...
        if (selectionArg.equals(UNSET_STRING)) selectionArgs.add("");
        else selectionArgs.add(substring ? ("%" + selectionArg + "%") : selectionArg);
      }

      // Get a WHERE clause (plus trailing space) if |selections| is non-empty, or just an empty
      // string otherwise.
    ...
    String query =
        "SELECT s.SongId, Artist, Title, Album, AlbumId, Url, CoverUrl, Length, "
            ...
            + "ORDER BY "
            + (shuffle ? "RANDOM()" : "Album ASC, AlbumId ASC, DiscNumber ASC, TrackNumber ASC")
            + " "
            + "LIMIT "
            + MAX_QUERY_RESULTS;
    Log.d(
  ...
  public boolean syncWithServer(SyncProgressListener listener, String message[]) {
    ...
    try {
      ...
      // starting point for
      // the next sync, to handle the case where some songs in the server are updated while we're
      // doing this sync.
  ...
  private int queryServer(
      SQLiteDatabase db,
      long prevStartTimeNsec,
      boolean deleted,
    ...
    while (numUpdates == 0 || !serverCursor.isEmpty()) {
      String path =
          String.format(
              ...
              prevStartTimeNsec, deleted ? 1 : 0, SERVER_SONG_BATCH_SIZE, serverCursor);
      ...
      try {
        ...
        for (int i = 0; i < objects.length(); ++i) {
          ...
          if (deleted) {
            ...
            values.put(
                "TrackGain", jsonSong.has("trackGain") ? jsonSong.getDouble("trackGain") : 0.0);
            values.put(
                "AlbumGain", jsonSong.has("albumGain") ? jsonSong.getDouble("albumGain") : 0.0);
      ...
      listener.onSyncProgress(
          deleted ? SyncState.DELETING_SONGS : SyncState.UPDATING_SONGS, numUpdates);
    }
  ...
  private void updateStatsTables(SQLiteDatabase db) {
    ...
    while (!cursor.isAfterLast()) {
      ...
      if (!artistCaseMap.containsKey(lowerArtist)) artistCaseMap.put(lowerArtist, origArtist);
