// Copyright 2011 Daniel Erat <dan@erat.org>
// All rights reserved.

package org.erat.nup;

public class NupPreferences {
    public static final String SERVER_URL = "server_url";
    public static final String USERNAME = "username";
    public static final String PASSWORD = "password";
    public static final String ACCOUNT = "account";
    public static final String SYNC_SONG_LIST = "sync_song_list";
    public static final String PRE_AMP_GAIN = "pre_amp_gain";
    public static final String CACHE_SIZE = "cache_size";
    public static final String CLEAR_CACHE = "clear_cache";
    public static final String SONGS_TO_PRELOAD = "songs_to_preload";
    public static final String DOWNLOAD_RATE = "download_rate";

    public static final String PRE_AMP_GAIN_DEFAULT = "0";
    public static final String CACHE_SIZE_DEFAULT = "512";
    public static final int CACHE_SIZE_MINIMUM = 20;
    public static final String SONGS_TO_PRELOAD_DEFAULT = "1";
    public static final String DOWNLOAD_RATE_DEFAULT = "0";
}
