...
public class SettingsActivity extends PreferenceActivity
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    int maxCacheMb =
        Integer.parseInt(
            mPrefs.getString(NupPreferences.CACHE_SIZE, NupPreferences.CACHE_SIZE_DEFAULT));
    ...
    pref.setSummary(
        getResources()
            .getQuantityString(R.plurals.songs_to_preload_fmt, songsToPreload, songsToPreload));

    pref = findPreference(NupPreferences.DOWNLOAD_RATE);
    pref.setOnPreferenceChangeListener(this);
    int downloadRate =
        Integer.parseInt(
            mPrefs.getString(NupPreferences.DOWNLOAD_RATE, NupPreferences.DOWNLOAD_RATE_DEFAULT));
    pref.setSummary(
        downloadRate == 0
  ...
  public boolean onPreferenceChange(Preference pref, Object value) {
    if (pref.getKey().equals(NupPreferences.SERVER_URL)) {
      ...
      try {
        ...
        findPreference(NupPreferences.PRE_AMP_GAIN)
            .setSummary(getString(R.string.pre_amp_gain_value, gain));
        return true;
      ...
      findPreference(NupPreferences.CACHE_SIZE)
          .setSummary(getString(R.string.cache_size_value, intValue));
      return true;
    } else if (pref.getKey().equals(NupPreferences.SONGS_TO_PRELOAD)) {
      int intValue = parseNonNegativeInt((String) value);
      if (intValue < 0) return false;
      findPreference(NupPreferences.SONGS_TO_PRELOAD)
          .setSummary(
              getResources().getQuantityString(R.plurals.songs_to_preload_fmt, intValue, intValue));
      return true;
    } else if (pref.getKey().equals(NupPreferences.DOWNLOAD_RATE)) {
      int intValue = parseNonNegativeInt((String) value);
      if (intValue < 0) return false;
      findPreference(NupPreferences.DOWNLOAD_RATE)
          .setSummary(
              intValue == 0
                  ? getString(R.string.unlimited)
