...
public class MediaSessionManager {
  ...
  public void updateSong(Song song) {
    ...
    if (song != null) {
      ...
      if (bitmap != null) {
        // Pass a copy of the original bitmap. Apparently the later apply() call recycles the
        // bitmap, which then
        // causes a crash when we try to use it later:
        // https://code.google.com/p/android/issues/detail?id=74967
        builder.putBitmap(
            MediaMetadata.METADATA_KEY_ALBUM_ART, bitmap.copy(bitmap.getConfig(), true));
      }
    }
