...
import org.erat.nup.R
import org.erat.nup.NotificationCreator
...
internal class NotificationCreator(
    ...
    fun createNotification(
            ...
            songIndex: Int,
            numSongs: Int): Notification? {
        val showPlayPause = song != null && !playbackComplete
        val showPrev = songIndex > 0
        val showNext = numSongs > 0 && songIndex < numSongs - 1
        if (onlyIfChanged
                && song != null && song.id == songId && paused == this.paused && song.coverBitmap != null == showingCoverBitmap && showPlayPause == showingPlayPause && showPrev == showingPrev && showNext == showingNext) {
            return null
        }
        songId = song?.id ?: 0
        this.paused = paused
        ...
        val builder = Notification.Builder(context)
                .setContentTitle(
                        song?.artist ?: context.getString(R.string.startup_message_title))
                .setContentText(
                        song?.title ?: context.getString(R.string.startup_message_text))
        ...
        if (song != null) {
            ...
            builder.style = style
            val numActions = (if (showPlayPause) 1 else 0) + (if (showPrev) 1 else 0) + if (showNext) 1 else 0
            val showLabels = numActions < 3
            if (showPlayPause) {
                builder.addAction(
                        if (paused) R.drawable.ic_play_arrow_black_36dp else R.drawable.ic_pause_black_36dp,
                        if (showLabels) context.getString(if (paused) R.string.play else R.string.pause) else "",
                        togglePauseIntent)
            }
            if (showPrev) {
                builder.addAction(
            ...
            if (numActions > 0) {
                ...
                style.setShowActionsInCompactView(
                        *if (numActions == 3) intArrayOf(0, 1, 2) else if (numActions == 2) intArrayOf(0, 1) else intArrayOf(0))
            }
        }
